...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        parser.add_argument('--logdir', '-r', type=list,
                            default=[
                                # windows路径示例:
                                "/home/robot/robot_code/baselines-master/Ablation_norm/fig6_other_tasks/dher_fix003_rand244/F",
                                "/home/robot/robot_code/baselines-master/Ablation_norm/fig6_other_tasks/base_her_19/F",

                            ])
        # other nargs
        parser.add_argument('--select', default=[], )
        # 移动的抽屉，DHER也不好学！
        parser.add_argument('--exclude', default=["Move"], )

    parser.add_argument('--title', '-t', nargs='*')
    parser.add_argument('--legend', '-l', nargs='*')
    parser.add_argument('--mini', '-m', type=int, default=45,
                        help='可以画图的最短数据长度')
    parser.add_argument('--xclip', '-xc', type=int, default=50,
