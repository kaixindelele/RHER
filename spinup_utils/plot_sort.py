        ...
        if 'progress.txt' in files:
            ...
            try:
                ...
                if 'exp_name' in config:
                    ...
                    exp_name = exp_name.replace('HER_TD3Torch', 'TD3')
                    exp_name = exp_name.replace('Fetch', '')
                    exp_name = exp_name.replace('Double', 'D')
                    exp_name = exp_name.replace('gamma', 'gm')
                    exp_name = exp_name.replace('rf-4e+06_un40_ath0_05_dth0_05', '')
                    exp_name = exp_name.replace('_done0_dish0', '')
                    exp_name = exp_name.replace('-rp0_0-rn-1_0-ng-4', '')
    ...
    for exp_name, roots in roots_names_dict.items():
        for root in roots:
            ...
            try:
                ...
                continue
            performance = 'TestSuccess' if 'TestSuccess' in exp_data else 'TestStage2Success'
    ...
    if len(sys.argv) > 1:
        ...
        parser.add_argument('--logdir', '-r', type=list,
                            default=[
                                ...
                                "/home/robot/robot_code/rher_multi_obj/tune_td3_reset_mujoco/TD3_test_seeds/2",
                                # Ubuntu路径示例：
                                # "plot_demo_files/2020",
                            ])
        # other nargs
        # parser.add_argument('--select', default=['Hopper'], )
        parser.add_argument('--select', default=['HalfCheetah'], )

        parser.add_argument('--exclude', default=[], )

    parser.add_argument('--title', '-t', nargs='*')
    parser.add_argument('--legend', '-l', nargs='*')
    parser.add_argument('--mini', '-m', type=int, default=30,
                        help='可以画图的最短数据长度')
