...
def plot_data(data, xaxis='Epoch', value="TestEpRet",
    ...
    #         datum[condition] = 'R-' + str(rank_value) + "-" + datum[condition]
    ...
    """
    # 画一条基准线!
    plt.hlines(0.95, 0, np.max(np.asarray(data[xaxis])), linestyles='--', colors='#4472C4', label="Success Rate: 95%")
    plt.hlines(0.90, 0, np.max(np.asarray(data[xaxis])), linestyles='--', colors='grey', label="Success Rate: 90%")
    # # 对图例legend也做一个排序，这样看起来更直观~
    handles, labels = plt.gca().get_legend_handles_labels()
    sorted_handles = []
    sorted_labels = []
    for index in range(len(handles)-2):
    ...
    sorted_handles.append(handles[-2])
    sorted_labels.append(labels[-2])
    sorted_handles.append(handles[-1])
    sorted_labels.append(labels[-1])

    plt.legend(sorted_handles, sorted_labels, loc=args.loc, labelspacing=0.2,
               ncol=1,
               handlelength=2,
               # mode="expand",
               borderaxespad=0.,
        ...
        if 'progress.txt' in files:
            ...
            try:
                ...
                if 'exp_name' in config:
                    ...
                    elif 'reset_stage1_un_a' in logdir:
                        exp_name += 'reset(ours)-'
                    elif 'reset' in logdir:
                        exp_name += 'reset-in-15-epoch-'
                    ...
                    if "DoublePush" in real_exp_name:
                        ...
                        exp_name += "TStack"
                    elif "FetctThreePush" in real_exp_name:
                        exp_name += "TPush"
...
def make_plots(all_logdirs, legend=None,
    ...
    for value in values:
        ...
        try:
            ...
            pass
        fig.savefig(all_logdirs[0] + 'Fig_2obj' + select_str + exclude_str + '.png',
                    bbox_inches='tight',
                    dpi=600)
        fig.savefig(all_logdirs[0] + 'Fig_2obj' + select_str + exclude_str + '.svg',
                    bbox_inches='tight',
                    dpi=600)
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        parser.add_argument('--logdir', '-r', type=list,
                            default=[
                                ...
                                # "/home/robot/robot_code/rher_multi_obj/tune_rher_last/RHER_TD3_1obj_opt50_seeds/2",
                                # "/home/robot/robot_code/rher_multi_obj/tune_rher_reset_ac/RHER_TD3_1obj_reset_seeds/2",
                                # '/home/robot/robot_code/rher_multi_obj/tune_rher_reset_ac/RHER_TD3_1obj_reset1535_seeds/2',
                                # '/home/robot/robot_code/rher_multi_obj/tune_rher_reset/RHER_TD3_1obj_reset_stage1_un_a_seeds/2',
                                "/home/robot/robot_code/rher_multi_obj/tune_rher_reset_guide_last/SRHER_TTD3_1obj/2",
                                # Ubuntu路径示例：
                                # "plot_demo_files/2020",
                            ])
        # other nargs
        # parser.add_argument('--select', default=['Pick', '40'], )
        parser.add_argument('--select', default=['Push'], )
    ...
    parser.add_argument('--mini', '-m', type=int, default=10,
                        help='可以画图的最短数据长度')
    parser.add_argument('--xclip', '-xc', type=int, default=1500,
