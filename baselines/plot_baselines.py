        ...
        if 'progress.csv' in files:
            ...
            try:
                ...
                if 'env_name' in config:
                    exp_name = config['env_name']
                if 'RHER' in logdir or 'rher' in logdir:
                    exp_name += '_RHER'
...
def main():
    ...
    if len(sys.argv) > 1:        
        ...
        parser.add_argument('--logdir', '-r', type=list,
                            default=[
                                "/tmp/rher_results/rher_np19_case0_results/Fetch",
                                "/tmp/rher_results/her_np19_results/Fetch",
                            ])
        title_str = 'rand' + str(rate)
        parser.add_argument('--title', default=title_str, )
        # other nargs
