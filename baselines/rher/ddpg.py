...
from baselines.rher.util import (
    import_function, store_args, flatten_grads, transitions_in_episode_batch, convert_episode_to_batch_major)
from baselines.rher.normalizer import Normalizer
from baselines.rher.replay_buffer import ReplayBuffer
...
class DDPG(object):
    ...
    def __init__(self, input_dims, buffer_size, hidden, layers, network_class, polyak, batch_size,
        ...
        Args:
            ...
            network_class (str): the network class that should be used (e.g. 'baselines.rher.ActorCritic')
        ...
        # print("self.rollout_batch_size:", self.rollout_batch_size)
    ...
    def check_reached(self, gg, ag, g, reward_func=None, th=0.14):
        ...
        if th < 0.05:
            ...
            if grip2obj > th and reward_func(gg, ag, None) == -1:
                reached = False
            else:
                reached = True
        return reached
    ...
    def get_actions(self, o, ag, g, noise_eps=0., random_eps=0., use_target_net=False,
                    compute_Q=False, rew_func=None, epoch=0, cycle=0, test_success_rate=0.0):
        rd = 0.2
        rr = 0.4 / (1.0 - rd)
        th = 0.03

        reached = self.check_reached(o[:, :3], ag, g=g.reshape(1, 3), reward_func=rew_func, th=th)
        # only train use reach mode
        stage_sep = True
        case = 0
        if stage_sep:
            if case == 0:
                ...
                if noise_eps > 0:
                    # train
                    if (np.random.random() < rr) and (not reached) and test_success_rate < 0.8:
                        reach_a = True
                        g = g * 0
                    else:
                        reach_a = False
                ...
                if noise_eps > 0:
                    ...
                    # test
                    if not reached and (np.random.random() < rr):
                        reach_a = True
                        g = g * 0
                    else:
                        reach_a = False
            ...
            if np.random.random() < rr and noise_eps > 0 and not reached:
                reach_a = True
                g = g * 0
            else:
                reach_a = False
        ...
        feed = {
            ...
            policy.ag_tf: ag.reshape(-1, self.dimag),
        ...
        if not reach_a and noise_eps > 0:
            u += np.random.binomial(1, rd, u.shape[0]).reshape(-1, 1) * (self._random_action(u.shape[0]) - u)  # eps-greedy
        if u.shape[0] == 1:
            u = u[0]
        u = u.copy()
        ret[0] = u
    ...
    def init_demo_buffer(self, demoDataFile, update_stats=True): #function that initializes the demo buffer
        ...
        for epsd in range(self.num_demo): # we initialize the whole demo buffer at the start of the training
            ...
            if update_stats:
                ...
                transitions['o'], transitions['ag'], transitions['g'] = self._preprocess_og(o, ag, g)
                # No need to preprocess the o_2 and g_2 since this is only used for stats

                self.o_stats.update(transitions['o'])
                self.g_stats.update(transitions['g'])
                self.ag_stats.update(transitions['ag'])
    ...
    def sample_batch(self, rekey='g'):
        if self.bc_loss: #use demonstration buffer to sample as well if bc_loss flag is set TRUE
            ...
            transitions = self.buffer.sample(self.batch_size, rekey=rekey) #otherwise only sample from primary buffer
        ...
        if rekey == 'g':
            ...
            g *= 0
        transitions['o'], transitions['ag'], transitions['g'] = self._preprocess_og(o, ag, g)
        transitions['o_2'], transitions['ag_2'], transitions['g_2'] = self._preprocess_og(o_2, ag_2, g)

        transitions_batch = [transitions[key] for key in self.stage_shapes.keys()]

        return transitions_batch

    def stage_batch(self, batch=None, rekey='g'):
        if batch is None:
            batch = self.sample_batch(rekey=rekey)
        assert len(self.buffer_ph_tf) == len(batch)
        self.sess.run(self.stage_op, feed_dict=dict(zip(self.buffer_ph_tf, batch)))

    def train(self, stage=True, rekey='g'):
        if stage:
            self.stage_batch(rekey=rekey)
    ...
    def _create_network(self, reuse=False):
        ...
        # 加入ag_stats!
        with tf.variable_scope('ag_stats') as vs:
            if reuse:
                vs.reuse_variables()
            self.ag_stats = Normalizer(self.dimag, self.norm_eps, self.norm_clip, sess=self.sess)
        ...
        self.stats_vars = self._global_vars('o_stats') + self._global_vars('g_stats') + self._global_vars('ag_stats')
    ...
    def logs(self, prefix=''):
        ...
        logs += [('stats_ag/mean', np.mean(self.sess.run([self.ag_stats.mean])))]
        logs += [('stats_ag/std', np.mean(self.sess.run([self.ag_stats.std])))]
