...
def plot_data(data, xaxis='Epoch', value="TestEpRet",
    ...
    """
    x_max = np.max(np.asarray(data[xaxis]))
    plt.hlines(0.95, 0, x_max, linestyles='--', colors='blue', label="Succuss Rate: 95%")
...
def make_plots(all_logdirs, legend=None,
    ...
    try:
        ...
        pass
    fig.savefig('Fig_push_mpi1.png',
                bbox_inches='tight',
                dpi=600)
    fig.savefig('Fig_push_d_mpi1.svg',
                bbox_inches='tight',
                dpi=600)
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        # 数据量少的路径放前面，否则会被clip_xaxis截断
        parser.add_argument('--logdir', '-r', type=list,
                            default=[
                                r".\data\fig_baseline\single_process\rher_d003_rate244_np1_results\F",
                                r".\data\fig_baseline\single_process\base_her_rd03_long_np1_results\F",
                            ])
        # other nargs
        parser.add_argument('--select', default=['Push'], )
        parser.add_argument('--exclude', default=['Slide'], )

    parser.add_argument('--title', '-t', nargs='*')
    parser.add_argument('--legend', '-l', nargs='*')
    parser.add_argument('--mini', '-m', type=int, default=45,
                        help='可以画图的最短数据长度')
    parser.add_argument('--xclip', '-xc', type=int, default=600,
